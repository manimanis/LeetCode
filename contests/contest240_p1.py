...
range [birthi, deathi - 1]. Note that the person is not counted in the 
year that they die.
...
class Solution:
    def maximumPopulation(self, logs: List[List[int]]) -> int:
        years = {y: 0 for y in range(1950, 2051)}
        mx = 1950
        for log in logs:
...
if __name__ == '__main__':
    make_tests(test_cases, Solution, 'maximumPopulation')